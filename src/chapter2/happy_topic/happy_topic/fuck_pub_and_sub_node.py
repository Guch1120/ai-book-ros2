...
class FuckyouPubAndSub(Node):
    def __init__(self):
        super().__init__('fuckyou_pun_and_sub')
        self.pub = self.create_publisher(String , 'fuckyou_msg' ,10)
    ...
    def callback(self, sub_msg):
        ...
        pub_msg.data = self.fuck_actions[sub_msg.data % 10 + 1]
        self.pub.publish(pub_msg)
        self.get_logger().info(f'パブリッシュ{pub_msg.data}')

def main():
    rclpy.init()
    node = FuckyouPubAndSub()
    try:
        rclpy.spin(node)
